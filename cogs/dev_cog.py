...
class ReloadButton(discord.ui.Button):
    def __init__(
        self, cog: str, all_cogs: list[str] | None = None, disable: bool = False
    ):
        super().__init__(
            label=cog,
            style=discord.ButtonStyle.primary,
            disabled=disable,
            custom_id=f"reload_{cog}_btn",
        )
        self.cog = cog
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            delete_after = 5
            embed = ui.dev_embed(
                f"已成功重新載入 `{'`, `'.join(cogs)}`。 共重新載入 {len(cogs)} 項內容。"
            )
            logging.info(
                f"已成功重新載入 `{'`, `'.join(cogs)}`。 共重新載入 {len(cogs)} 項內容。"
            )
            ...
            await interaction.edit_original_response(
                embed=embed, view=get_reload_all_view()
            )
...
class DevCog(commands.Cog):
    ...
    async def load(self, ctx: commands.Context, extension):
        ...
        try:
            ...
            embed = ui.dev_embed(
                f"**已成功載入 `{'`, `'.join(cogs)}`。 共載入 {len(cogs)} 項內容。**"
            )
            logging.info(
                f"已成功載入 `{'`, `'.join(cogs)}`。 共載入 {len(cogs)} 項內容。"
            )
    ...
    async def unload(self, ctx: commands.Context, extension):
        ...
        try:
            ...
            embed = ui.dev_embed(
                f"**已成功解除載入 `{'`, `'.join(cogs)}`。 共解除載入 {len(cogs)} 項內容。**"
            )
            logging.info(
                f"已成功解除載入 `{'`, `'.join(cogs)}`。 共解除載入 {len(cogs)} 項內容。"
            )
    ...
    async def reload(self, ctx: commands.Context, extension: str | None = None):
        ...
        if extension is None:
            await ctx.reply(
                embed=ui.dev_embed("請選擇要重新載入的模組"),
                view=get_reload_all_view(),
                ephemeral=True,
            )
            return
        try:
...
class CogFileChangeHandler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        if (
            ...
            and str(event.src_path).endswith(".py")
            and (
                self.last_modified != event.src_path
                or current_time - self.last_modified_time > 2
            )
        ):
            self.last_modified = event.src_path
...
async def setup(bot: commands.Bot):
    ...
    if settings.AUTO_RELOAD:
        dev_cog.file_watcher_task = bot.loop.create_task(
            start_file_watcher(bot, dev_cog)
        )
